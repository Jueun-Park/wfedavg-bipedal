...
def ho_save_client(base_index, subenv_seed):
    ...
    learner.env = env
    learner.verbose = 0
    callback = SaveRNDDatasetCallback(base_index=base_index, subenv_seed=subenv_seed)
    learner.learn(total_timesteps=client_timesteps,
                  callback=callback,
                  )

    dir_name = f"ho_base{base_index}_client_model/{subenv_seed}"
...
if __name__ == "__main__":
    args_list = []
    for args in product(range(4), [seed + i for i in range(4)]):
        args_list.append(args)

    start_time = time.time()
    with mp.Pool(None) as p:
        p.starmap(ho_save_client, args_list)
